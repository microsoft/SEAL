...
in Microsoft SEAL. See more explanation of the compression mechanism in [README.md](README.md#zlib).
Microsoft SEAL does not redistribute ZLIB and this feature must be enabled by the user.
...
object starts with a 16-byte `Serialization::SEALHeader` struct, and is followed by the data for
...
old serialized Microsoft SEAL objects are incompatible with the new format, but it should be very
easy to create a conversion utility if needed.
...
installed on the same system easily, as the default install directory and library filename now
