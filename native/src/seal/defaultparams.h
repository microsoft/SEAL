...
namespace seal
{
    ...
    */
    ...
    public:
        ...
        /**
        For a given polynomial modulus degree:
        if coeff_modulus_count is given, choose the maximum 128-bit secure coeff
        ...
        @throws std::out_of_range if poly_modulus_degree is not 1024, 2048, 4096, 8192,
            16384, or 32768
        */
        inline static std::vector<SmallModulus> coeff_modulus_128(
            std::size_t poly_modulus_degree, std::size_t coeff_modulus_count = 0)
        {
            if (coeff_modulus_count == 0)
            {
                try
                {
                    return util::global_variables::default_coeff_modulus_128.
        ...
        */
        ...
        {
            std::sort(coeff_modulus.begin(), coeff_modulus.end(), std::greater<SmallModulus>());
