...
namespace seal
{
    Decryptor::Decryptor(shared_ptr<SEALContext> context, const SecretKey &secret_key) : context_(move(context))
    {
        ...
        set_poly_poly(secret_key.data().data(), coeff_count, coeff_mod_count, secret_key_array_.get());
    ...
    void Decryptor::compute_secret_key_array(size_t max_power)
    {
        ...
        auto new_secret_key_array(allocate_poly(mul_safe(new_size, coeff_count), coeff_mod_count, pool_));
        set_poly_poly(secret_key_array_.get(), old_size * coeff_count, coeff_mod_count, new_secret_key_array.get());

        set_poly_poly(
            secret_key_array_.get(), mul_safe(old_size, coeff_count), coeff_mod_count, new_secret_key_array.get());
    ...
    int Decryptor::invariant_noise_budget(const Ciphertext &encrypted)
    {
        ...
        poly_infty_norm_coeffmod(
            noise_poly.get(), coeff_count, coeff_mod_count, context_data.total_coeff_modulus(), norm.get(), pool_);
