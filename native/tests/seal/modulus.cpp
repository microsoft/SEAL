...
namespace SEALTest
{
    TEST(CoeffModTest, CustomExceptionTest)
    {
        ...
        ASSERT_THROW(CoeffModulus::Custom(1, { 2 }), invalid_argument);

        // Too large poly_modulus_degree
        ASSERT_THROW(CoeffModulus::Custom(65536, { 30 }), invalid_argument);

        // Invalid poly_modulus_degree
        ASSERT_THROW(CoeffModulus::Custom(1023, { 20 }), invalid_argument);

        // Invalid bit-size
        ASSERT_THROW(CoeffModulus::Custom(2048, { 0 }), invalid_argument);
        ASSERT_THROW(CoeffModulus::Custom(2048, { -30 }), invalid_argument);
        ASSERT_THROW(CoeffModulus::Custom(2048, { 30, -30 }), invalid_argument);

        // Too small primes requested
        ASSERT_THROW(CoeffModulus::Custom(2, { 2 }), logic_error);
        ASSERT_THROW(CoeffModulus::Custom(2, { 3, 3, 3 }), logic_error);
        ASSERT_THROW(CoeffModulus::Custom(1024, { 8 }), logic_error);
    }

    TEST(CoeffModTest, CustomTest)
    {
        auto cm = CoeffModulus::Custom(2, { });
        ASSERT_EQ(0, cm.size());

        cm = CoeffModulus::Custom(2, { 3 });
        ASSERT_EQ(1, cm.size());
        ASSERT_EQ(uint64_t(5), cm[0].value());

        cm = CoeffModulus::Custom(2, { 3, 4 });
        ASSERT_EQ(2, cm.size());
        ASSERT_EQ(uint64_t(5), cm[0].value());
        ASSERT_EQ(uint64_t(13), cm[1].value());

        cm = CoeffModulus::Custom(2, { 3, 5, 4, 5 });
        ASSERT_EQ(4, cm.size());
        ASSERT_EQ(uint64_t(5), cm[0].value());
        ...
        cm = CoeffModulus::Custom(32, { 30, 40, 30, 30, 40 });
        ASSERT_EQ(5, cm.size());
